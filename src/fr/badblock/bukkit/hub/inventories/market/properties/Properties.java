package fr.badblock.bukkit.hub.inventories.market.properties;

import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.BatCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.BlazeCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.CaveSpiderCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.ChickenCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.CowCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.CreeperCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.EndermanCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.GhastCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.GiantCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.GuardianCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.HorseCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.IronGolemCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.MagmaCubeCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.OcelotCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.PigCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.PigZombieCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.RabbitCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.SheepCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.SilverfishCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.SkeletonCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.SlimeCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.SnowmanCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.SpiderCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.SquidCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.VillagerCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.WitchCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.WitherSkullCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.WolfCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.disguises.submenu.data.creatures.ZombieCreatureMetamorphosisItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.BatMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.BlazeMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.BunnyMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.CaveSpiderMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.ChickenMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.CowMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.CreeperMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.EndermanMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.GhastMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.GiantMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.GuardianMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.IronGolemMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.MagmaCubeMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.MushroomCowMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.OcelotMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.PegasusMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.PigMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.SheepMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.SilverfishMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.SkeletonMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.SlimeMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.SnowmanMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.SpiderMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.SquidMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.VillagerMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.WitchMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.WolfMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.mounts.ZombieMountItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.AnimatedBallParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.ArcParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.AtomParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.BleedParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.CircleParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.CloudParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.ColoredImageParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.ConeParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.CubeParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.CylinderParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.DiscoBallParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.DnaParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.DonutParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.DragonParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.EarthParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.EquationParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.FlameParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.FountainParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.GridParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.HeartParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.HelixParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.HillParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.IconParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.ImageParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.JumpParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.LineParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.LoveParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.MusicParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.ShieldParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.SkyRocketParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.SmokeParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.SphereParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.StarParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.TextParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.TornadoParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.TraceParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.TurnParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.VortexParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.WarpParticleItem;
import fr.badblock.bukkit.hub.inventories.market.cosmetics.particles.data.WaveParticleItem;
import fr.badblock.bukkit.hub.inventories.market.ownitems.OwnableItem;
import lombok.Getter;

@Getter public enum Properties {

	DISGUISES_BAT(new BatCreatureMetamorphosisItem()),
	DISGUISES_BLAZE(new BlazeCreatureMetamorphosisItem()),
	DISGUISES_CAVESPIDER(new CaveSpiderCreatureMetamorphosisItem()),
	DISGUISES_CHICKEN(new ChickenCreatureMetamorphosisItem()),
	DISGUISES_COW(new CowCreatureMetamorphosisItem()),
	DISGUISES_CREEPER(new CreeperCreatureMetamorphosisItem()),
	DISGUISES_ENDERMAN(new EndermanCreatureMetamorphosisItem()),
	DISGUISES_GHAST(new GhastCreatureMetamorphosisItem()),
	DISGUISES_GIANT(new GiantCreatureMetamorphosisItem()),
	DISGUISES_GUARDIAN(new GuardianCreatureMetamorphosisItem()),
	DISGUISES_HORSE(new HorseCreatureMetamorphosisItem()),
	DISGUISES_IRONGOLEM(new IronGolemCreatureMetamorphosisItem()),
	DISGUISES_MAGMACUBE(new MagmaCubeCreatureMetamorphosisItem()),
	DISGUISES_MUSHROOMCOW(new MagmaCubeCreatureMetamorphosisItem()),
	DISGUISES_OCELOT(new OcelotCreatureMetamorphosisItem()),
	DISGUISES_PIG(new PigCreatureMetamorphosisItem()),
	DISGUISES_PIGZOMBIE(new PigZombieCreatureMetamorphosisItem()),
	DISGUISES_RABBIT(new RabbitCreatureMetamorphosisItem()),
	DISGUISES_SHEEP(new SheepCreatureMetamorphosisItem()),
	DISGUISES_SILVERFISH(new SilverfishCreatureMetamorphosisItem()),
	DISGUISES_SKELETON(new SkeletonCreatureMetamorphosisItem()),
	DISGUISES_SLIME(new SlimeCreatureMetamorphosisItem()),
	DISGUISES_SNOWMAN(new SnowmanCreatureMetamorphosisItem()),
	DISGUISES_SPIDER(new SpiderCreatureMetamorphosisItem()),
	DISGUISES_SQUID(new SquidCreatureMetamorphosisItem()),
	DISGUISES_VILLAGER(new VillagerCreatureMetamorphosisItem()),
	DISGUISES_WITCH(new WitchCreatureMetamorphosisItem()),
	DISGUISES_WITHERSKULL(new WitherSkullCreatureMetamorphosisItem()),
	DISGUISES_WOLF(new WolfCreatureMetamorphosisItem()),
	DISGUISES_ZOMBIE(new ZombieCreatureMetamorphosisItem()),
	MOUNTS_BAT(new BatMountItem()),
	MOUNTS_BLAZE(new BlazeMountItem()),
	MOUNTS_BUNNY(new BunnyMountItem()),
	MOUNTS_CAVESPIDER(new CaveSpiderMountItem()),
	MOUNTS_CHICKEN(new ChickenMountItem()),
	MOUNTS_COW(new CowMountItem()),
	MOUNTS_CREEPER(new CreeperMountItem()),
	MOUNTS_ENDERMAN(new EndermanMountItem()),
	MOUNTS_GHAST(new GhastMountItem()),
	MOUNTS_GIANT(new GiantMountItem()),
	MOUNTS_GUARDIAN(new GuardianMountItem()),
	MOUNTS_IRONGOLEM(new IronGolemMountItem()),
	MOUNTS_MAGMACUBE(new MagmaCubeMountItem()),
	MOUNTS_MUSHROOMCOW(new MushroomCowMountItem()),
	MOUNTS_OCELOT(new OcelotMountItem()),
	MOUNTS_PEGASUS(new PegasusMountItem()),
	MOUNTS_PIG(new PigMountItem()),
	MOUNTS_PIGZOMBIE(new SheepMountItem()),
	MOUNTS_SHEEP(new SheepMountItem()),
	MOUNTS_SILVERFISH(new SilverfishMountItem()),
	MOUNTS_SKELETON(new SkeletonMountItem()),
	MOUNTS_SLIME(new SlimeMountItem()),
	MOUNTS_SNOWMAN(new SnowmanMountItem()),
	MOUNTS_SPIDER(new SpiderMountItem()),
	MOUNTS_SQUID(new SquidMountItem()),
	MOUNTS_VILLAGER(new VillagerMountItem()),
	MOUNTS_WITCH(new WitchMountItem()),
	MOUNTS_WOLF(new WolfMountItem()),
	MOUNTS_ZOMBIE(new ZombieMountItem()),
	PARTICLES_ANIMATEDBALL(new AnimatedBallParticleItem()),
	PARTICLES_ARC(new ArcParticleItem()),
	PARTICLES_ATOM(new AtomParticleItem()),
	PARTICLES_BLEED(new BleedParticleItem()),
	PARTICLES_CIRCLE(new CircleParticleItem()),
	PARTICLES_CLOUD(new CloudParticleItem()),
	PARTICLES_COLOREDIMAGE(new ColoredImageParticleItem()),
	PARTICLES_CONE(new ConeParticleItem()),
	PARTICLES_CUBE(new CubeParticleItem()),
	PARTICLES_CYLINDER(new CylinderParticleItem()),
	PARTICLES_DISCOBALL(new DiscoBallParticleItem()),
	PARTICLES_DNA(new DnaParticleItem()),
	PARTICLES_DONUT(new DonutParticleItem()),
	PARTICLES_DRAGON(new DragonParticleItem()),
	PARTICLES_EARTH(new EarthParticleItem()),
	PARTICLES_EQUATION(new EquationParticleItem()),
	PARTICLES_FLAME(new FlameParticleItem()),
	PARTICLES_FOUNTAIN(new FountainParticleItem()),
	PARTICLES_GRID(new GridParticleItem()),
	PARTICLES_HEART(new HeartParticleItem()),
	PARTICLES_HELIX(new HelixParticleItem()),
	PARTICLES_HILL(new HillParticleItem()),
	PARTICLES_ICON(new IconParticleItem()),
	PARTICLES_IMAGE(new ImageParticleItem()),
	PARTICLES_JUMP(new JumpParticleItem()),
	PARTICLES_LINE(new LineParticleItem()),
	PARTICLES_LOVE(new LoveParticleItem()),
	PARTICLES_MUSIC(new MusicParticleItem()),
	PARTICLES_SHIELD(new ShieldParticleItem()),
	PARTICLES_SKYROCKET(new SkyRocketParticleItem()),
	PARTICLES_SMOKE(new SmokeParticleItem()),
	PARTICLES_SPHERE(new SphereParticleItem()),
	PARTICLES_STAR(new StarParticleItem()),
	PARTICLES_TEXT(new TextParticleItem()),
	PARTICLES_TORNADO(new TornadoParticleItem()),
	PARTICLES_TRACE(new TraceParticleItem()),
	PARTICLES_TURN(new TurnParticleItem()),
	PARTICLES_VORTEX(new VortexParticleItem()),
	PARTICLES_WARP(new WarpParticleItem()),
	PARTICLES_WAVE(new WaveParticleItem());
	
	private OwnableItem ownableItem;
	
	Properties(OwnableItem ownableItem) {
		this.ownableItem = ownableItem;
	}
	
	public static Properties get(String name) {
		for (Properties property : values())
			if (property.name().toLowerCase().equals(name.toLowerCase())) return property;
		return null;
	}
	
}
