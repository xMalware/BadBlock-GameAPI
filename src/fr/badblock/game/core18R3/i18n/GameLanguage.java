...
public class GameLanguage implements Language {
	...
	/*
	 * protected GameLanguageFile getLanguageFile(String file){ file =
	 * file.toLowerCase();
	 * 
	 * if(!files.containsKey(file)){ return files.put(file, new
	 * GameLanguageFile(new File(textFolder, file + ".json"),
	 * getMessageWhenUnknow())); } else { return files.get(file); } }
	 */
	...
	public Message getMessage(String key) {
		 ...
		 * 
		 * if(file == null){ file = new GameLanguageFile(new File(textFolder,
		 * splitted[0].toLowerCase() + ".json"), getMessageWhenUnknow());
