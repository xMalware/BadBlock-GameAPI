...
public class FakeEntityTracker {
	static class FakeEntityTrackerEntry {
		...
		boolean isVisibleFrom(Location location) {
			...
			return dX <= TRACKING_RANGE && dZ <= TRACKING_RANGE
		...
		void remove(BadblockPlayer player) {
			...
			GameAPI.getAPI().createPacket(PlayEntityDestroy.class).setEntities(new int[] { fakeEntity.getId() })
					.send(player);
		}
		...
		void updatePlayer(BadblockPlayer player) {
			...
			if (player == null || fakeEntity.isRemoved())
				return;
	...
	public void doTick() {
		...
		Set<BadblockPlayer> players = trackedWorld.getPlayers().stream().map(player -> {
			return (BadblockPlayer) player;
		}).collect(Collectors.toSet());
		trackedEntities.stream().filter(entity -> {
			return entity.update(players);
		}).forEach(entity -> trackedEntities.remove(entity));
	}
