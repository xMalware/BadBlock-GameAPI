...
public abstract class GameFakeEntity<T extends WatcherEntity> implements FakeEntity<T> {
	...
	@Getter
	private List<UUID> viewingPlayers;
	@Getter
	...
	@Getter
	private FakeEntityTrackerEntry entry;
	...
	@Override
	public void kill() {
		broadcastPacket(GameAPI.getAPI().createPacket(PlayEntityStatus.class).setEntityId(id)
	...
	public void move(Location location) {
		...
		if (dX < 4 && dY < 4 && dZ < 4) {
			broadcastPacket(
					GameAPI.getAPI().createPacket(PlayEntityRelativeMove.class)
							.setEntityId(id).setMove(new Vector(location.getX() - oldLoc.getX(),
									location.getY() - oldLoc.getY(), location.getZ() - oldLoc.getZ()))
							.setOnGround(true));
			yaw(location);
		} else
			teleport(location);
	...
	public void playStatus(EntityStatus status) {
		broadcastPacket(GameAPI.getAPI().createPacket(PlayEntityStatus.class).setEntityId(id).setStatus(status));
	}
	...
	public void setEquipment(EquipmentSlot equipmentSlot, ItemStack itemstack) {
		...
		broadcastPacket(GameAPI.getAPI().createPacket(PlayEntityEquipment.class).setEntityId(id)
				.setSlot(equipmentSlot.ordinal()).setItemStack(itemstack));
	}

	@Override
	public void setHeadYaw(float yaw) {
		broadcastPacket(GameAPI.getAPI().createPacket(PlayEntityHeadRotation.class).setEntityId(id).setRotation(yaw));
		this.headYaw = yaw;
	}
	...
	public void show0(BadblockPlayer player) {
		...
		GameAPI.getAPI().createPacket(PlayEntityMetadata.class).setEntityId(id).setWatcher(watchers).send(player);

		if (headYaw != 0)
			GameAPI.getAPI().createPacket(PlayEntityHeadRotation.class).setEntityId(id).setRotation(headYaw)
					.send(player);

		for (EquipmentSlot slot : EquipmentSlot.values()) {
			if (equipment.containsKey(slot)) {
				GameAPI.getAPI().createPacket(PlayEntityEquipment.class).setEntityId(id).setSlot(slot.ordinal())
						.setItemStack(equipment.get(slot)).send(player);
			}
		}
	}
	...
	public void teleport(Location location) {
		broadcastPacket(GameAPI.getAPI().createPacket(PlayEntityTeleport.class).setEntityId(id).setTo(location)
				.setOnGround(true));
		yaw(location);
	...
	public void updateWatchers() {
		broadcastPacket(GameAPI.getAPI().createPacket(PlayEntityMetadata.class).setEntityId(id).setWatcher(watchers));
	}

	protected void yaw(Location location) {
		broadcastPacket(GameAPI.getAPI().createPacket(PlayEntityLook.class).setEntityId(id)
				.setPitch(location.getPitch()).setYaw(location.getYaw()));
		setHeadYaw(0);
	}
}
