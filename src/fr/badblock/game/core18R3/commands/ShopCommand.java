...
public class ShopCommand extends AbstractCommand {
	...
	public boolean executeCommand(CommandSender sender, String[] args) {
		...
		if(args[0].equals("edit")){
			...
			if(!merchant.getEntities().containsKey(args[2].toLowerCase())){
				...
				for(int i=0;i<merchant.getHandlers().size();i++){
					if(merchant.getHandlers().get(i).getName().equalsIgnoreCase(args[2])){
						merchant.getHandlers().remove(i); break;
					}
			...
			player.sendTranslatedMessage("commands.badshops.handlers", args[1], StringUtils.join(get(args[1]).getEntities().keySet(), ", "));
		} else return false;
	...
	private Merchant get(String name){
		...
		if(!file.exists()){
			...
			return merchant;
		} else return new Merchant(name.toLowerCase(), GameAPI.getAPI().loadConfiguration(file));
	}
}
