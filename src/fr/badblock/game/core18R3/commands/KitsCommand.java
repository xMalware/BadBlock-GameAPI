...
public class KitsCommand extends AbstractCommand {
	...
	public boolean executeCommand(CommandSender sender, String[] args) {
		...
		File folder = new File(GamePlugin.getInstance().getDataFolder() + File.separator + FOLDER_KITS,
				GameAPI.getInternalGameName());

		if (!folder.exists()) {
			player.sendTranslatedMessage("commands.kits.no-game", args[0]);
			return false;
		}
		...
		if (args[1].equalsIgnoreCase("create")) {
			...
			kit = new GameKit(args[0], false, Material.DIAMOND_SWORD.name(), (short) 0,
					new KitLevel[] { new KitLevel(new String[0], 100,
							GameAPI.getAPI().getKitContentManager().createFromInventory(player)) });
			...
			levels.add(new KitLevel(new String[0], 100,
					GameAPI.getAPI().getKitContentManager().createFromInventory(player)));
			...
			if (args.length == 2) {
				player.sendTranslatedMessage("commands.kits.usage-level");
				return true;
			}
			...
			try {
				...
				player.sendTranslatedMessage("commands.kits.unknow-level", args[2]);
				return true;
			}
