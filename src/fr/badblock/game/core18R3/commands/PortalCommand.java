...
public class PortalCommand extends AbstractCommand {
	...
	public boolean executeCommand(CommandSender sender, String[] args) {
		...
		case "list":
			concerned.sendTranslatedMessage("portals.command.list",
					StringUtils.join(GamePlugin.getInstance().getPortals().keySet(), ", "));
			break;
		case "create":
			if (args.length == 1)
				return false;
			if (concerned.getSelection() == null) {
				concerned.sendTranslatedMessage("commands.noselection",
						StringUtils.join(GamePlugin.getInstance().getPortals().keySet(), ", "));
				return true;
			}
