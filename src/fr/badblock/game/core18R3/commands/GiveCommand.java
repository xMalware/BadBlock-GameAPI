...
public class GiveCommand extends AbstractCommand {
	...
	public boolean executeCommand(CommandSender sender, String[] args) {
		...
		if(material == null){
		    Item nmsItem = (Item) Item.REGISTRY.get( new MinecraftKey(args[1]) );
		...
		int notGived = concerned.getInventory().addItem(result).values().stream().mapToInt(item -> {return item.getAmount();}).sum();
		
		if(notGived == 0){
			sendTranslatedMessage(sender, "commands.give.notgived", GameMessages.material(material, notGived > 0, WordDeterminant.SIMPLE), notGived, amount - notGived);
		} else {
			sendTranslatedMessage(sender, "commands.give.gived", GameMessages.material(material, notGived > 0, WordDeterminant.SIMPLE), amount);
		}
