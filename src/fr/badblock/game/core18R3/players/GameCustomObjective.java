...
public class GameCustomObjective implements CustomObjective {
	...
	enum Identifiers {
		line1('0', 1), line2('1', 2), line3('2', 3), line4('3', 4), line5('4', 5), line6('5', 6), line7('6', 7), line8(
				'7', 8), line9('8', 9), line10('9',
						10), line11('a', 11), line12('b', 12), line13('c', 13), line14('d', 14), line15('e', 15);
	...
	public GameCustomObjective(String name) {
		...
		for (int i = 1; i <= 15; i++) { // on prépare toutes les teams, inutile
										// de les unregisters elles ne gènent
										// pas :3
	...
	protected void changeTeam(BadblockPlayer player, CustomTeam team) {
		GameAPI.getAPI().createPacket(PlayScoreboardTeam.class).setPrefix(team.prefix).setSuffix(team.suffix)
				.setTeamName(team.name).setMode(TeamMode.UPDATE).send(player);
	}
	...
	public void removeScore(BadblockPlayer player, String offlinePlayer) {
		GameAPI.getAPI().createPacket(PlayScoreboardScore.class).setMode(ScoreMode.REMOVE).setObjectiveName(name)
				.setScoreName(offlinePlayer).send(player);
	}
	...
	protected void sendTeam(BadblockPlayer player, CustomTeam team) {
		if (Bukkit.getScoreboardManager().getMainScoreboard().getTeam(team.name) == null) { // la
																							// team
																							// existe
																							// déjà
																							// on
																							...
																							// le
																							// 'joueur'
			GameAPI.getAPI().createPacket(PlayScoreboardTeam.class).setTeamName(team.name)
					.setPlayers(new String[] { team.name }).setMode(TeamMode.ADD_PLAYERS).send(player);
		}

		GameAPI.getAPI().createPacket(PlayScoreboardTeam.class).setPrefix(team.prefix).setSuffix(team.suffix)
				.setTeamName(team.name).setPlayers(new String[] { team.name }).setMode(TeamMode.CREATE).send(player);
	}
	...
	public void setDisplayName(String displayName) {
		...
		GameAPI.getAPI().createPacket(PlayScoreboardObjective.class).setDisplayName(this.displayName)
				.setMode(ObjectiveMode.UPDATE).setObjectiveName(name).setObjectiveType(ObjectiveType.INTEGER)
				.send(getAssignedPlayer());
	}

	public void setScore(BadblockPlayer player, String offlinePlayer, int score) {
		GameAPI.getAPI().createPacket(PlayScoreboardScore.class).setMode(ScoreMode.CHANGE).setObjectiveName(name)
				.setScoreName(offlinePlayer).setScore(score).send(player);
	}

	@Override
	public void showObjective(BadblockPlayer player) {
		this.player = player.getUniqueId();
		((GameBadblockPlayer) player).setCustomObjective(this); // on donne à la
																// classe l'info

		GameAPI.getAPI().createPacket(PlayScoreboardObjective.class).setDisplayName(displayName)
				.setMode(ObjectiveMode.CREATE).setObjectiveName(name).setObjectiveType(ObjectiveType.INTEGER)
				.send(player);
		...
		GameAPI.getAPI().createPacket(PlayScoreboardDisplayObjective.class).setObjectiveName(name)
				.setObjectivePosition(ObjectivePosition.SIDEBAR).send(player);
	}
}
