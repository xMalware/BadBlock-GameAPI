...
public class GameJoinItems extends BadListener implements JoinItems {
	...
	private boolean clear = true, ended = false;
	...
	public void onPlayerDataLoaded(PlayerLoadedEvent e) {
		...
		if (kitItemSlot != -1) {
			...
			ItemEvent event = new ItemEvent() {
				...
				public boolean call(ItemAction action, BadblockPlayer player) {
					CustomInventory inventory = GameAPI.getAPI().createCustomInventory(kits.size() / 9,
							GameAPI.i18n().get(locale, "joinitems.kit.inventoryName")[0]);
			...
			ItemStack item = GameAPI.getAPI().createItemStackFactory().type(Material.DIAMOND_SWORD).doWithI18n(locale)
					.displayName("joinitems.kit.displayname").lore("joinitems.kit.lore").asExtra(1)
					.listenAs(event, ItemPlaces.HOTBAR_CLICKABLE).getHandler();
		...
		if (achieItemSlot != -1) {
			...
			ItemStack item = GameAPI.getAPI().createItemStackFactory().type(Material.NETHER_STAR).doWithI18n(locale)
					.displayName("joinitems.achievements.displayname").lore("joinitems.achievements.lore").asExtra(1)
					.listenAs(event, ItemPlaces.HOTBAR_CLICKABLE).getHandler();
		...
		if (teamItemSlot != -1) {
			ItemEvent event = new ItemEvent() {
				@Override
				public boolean call(ItemAction action, BadblockPlayer player) {
					CustomInventory inventory = GameAPI.getAPI().createCustomInventory(teams.size() / 9,
							GameAPI.i18n().get(locale, "joinitems.team.inventoryName")[0]);
			...
			ItemStack item = GameAPI.getAPI().createItemStackFactory().type(Material.WOOL).doWithI18n(locale)
					.displayName("joinitems.team.displayname").lore("joinitems.team.lore").asExtra(1)
					.listenAs(event, ItemPlaces.HOTBAR_CLICKABLE).getHandler();
		...
		if (voteItemSlot != -1) {
			...
			ItemStack item = GameAPI.getAPI().createItemStackFactory().type(Material.PAPER).doWithI18n(locale)
					.displayName("joinitems.vote.displayname").lore("joinitems.vote.lore").asExtra(1)
					.listenAs(event, ItemPlaces.HOTBAR_CLICKABLE).getHandler();
		...
		if (leaveItemSlot != -1) {
			...
			ItemStack item = GameAPI.getAPI().createItemStackFactory().type(Material.DARK_OAK_DOOR_ITEM)
					.doWithI18n(locale).displayName("joinitems.leave.displayname").lore("joinitems.leave.lore")
					.asExtra(1).listenAs(event, ItemPlaces.HOTBAR_CLICKABLE).getHandler();
