...
public class GameScoreboard extends BadListener implements BadblockScoreboard {
	...
	public void doGroupsPrefix() {
		...
		PermissionManager.getInstance().getGroups().forEach((group) -> {
			...
			// On ne d�finit pas le pr�fixe car il faut le faire pour chaque
			// joueur (pr�fixe alli� / ennemi)
	...
	@Override
	public void endVote() {
		if (votes == null || voteObjective == null)
			return;
		...
		if (doGroupsPrefix) {
			...
			if (player.getTeam() != null) {
				return new TranslatableString("game.customname", player.getName(),
						player.getTeam().getTabPrefix(ChatColor.GRAY));
			}
		}
	...
	public void onQuit(PlayerQuitEvent e) {
		...
		if (data.getElement() != null && votes.containsKey(data.getElement()) && voteObjective != null) {
			...
			voteObjective.getScore(GameAPI.i18n().replaceColors(data.getElement().getDisplayName()))
					.setScore(votes.get(data.getElement()));
		}

		if (getHandler().getEntryTeam(player.getName()) != null
				&& GameAPI.getAPI().getGameServer().getGameState() != GameState.RUNNING) {
	...
	public void openVoteInventory(BadblockPlayer player) {
		...
		CustomInventory inventory = GameAPI.getAPI().createCustomInventory(lines,
				i18n.get(locale, "vote.inventoryDisplayname")[0]);
		...
		for (Entry<VoteElement, Integer> entry : votes.entrySet()) {
			...
			inventory.addClickableItem(slot, GameAPI.getAPI().createItemStackFactory().type(Material.PAPER)
					.displayName(entry.getKey().getDisplayName())
					.lore(i18n.get(locale, "maps." + entry.getKey().getInternalName(), entry.getValue())).create(1),
					new ItemEvent() {
						@Override
						public boolean call(ItemAction action, BadblockPlayer player) {
							...
							if (data.getElement() != null && votes.containsKey(data.getElement())) {
								votes.put(data.getElement(), votes.get(data.getElement()) - 1);
								voteObjective.getScore(i18n.replaceColors(data.getElement().getDisplayName()))
										.setScore(votes.get(data.getElement()));
							}
	...
	/*
	 * Envoit le nouveau pr�fixe via packet, pour l'avoir custom sans le
	 * d�clarer dans un scoreboard personnel (opti)
	 */
	...
	private void sendTeamData(String teamName, String displayName, BadblockPlayer p) {
		GameAPI.getAPI().createPacket(PlayScoreboardTeam.class).setMode(TeamMode.UPDATE).setTeamName(teamName)
				.setFriendlyFire(TeamFriendlyFire.OFF).setPrefix(displayName).setDisplayName("")
				.setSuffix(ChatColor.RESET + (tabListHealth == null ? "" : " ")).setColor((byte) 0)
				.setNametagVisibility(TeamNameTag.always).send(p);
	}

	protected void sendTeams(BadblockPlayer player) {
		for (PermissibleGroup group : PermissionManager.getInstance().getGroups()) {
			sendTeamData(group.getName(),
					new TranslatableString("permissions.tab." + group.getName()).getAsLine(player), player);
		}
	}
}
