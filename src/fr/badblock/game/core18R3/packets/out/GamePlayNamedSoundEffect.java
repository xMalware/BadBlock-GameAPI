...
public class GamePlayNamedSoundEffect extends GameBadblockOutPacket implements PlayNamedSoundEffect {
	...
	public GamePlayNamedSoundEffect(PacketPlayOutNamedSoundEffect packet){
		...
		try {
			...
			position  = new Vector3f(
						(double) reflector.getFieldValue("b") / 8.0D,
						(double) reflector.getFieldValue("c") / 8.0D,
						(double) reflector.getFieldValue("d") / 8.0D
					);
	...
	public Packet<?> buildPacket() throws Exception {
		return new PacketPlayOutNamedSoundEffect(soundName, position.getX(), position.getY(), position.getZ(), volume, ((float) pitch) / 63.0F);
	}
