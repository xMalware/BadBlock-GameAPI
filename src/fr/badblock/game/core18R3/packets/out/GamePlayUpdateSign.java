...
public class GamePlayUpdateSign extends GameBadblockOutPacket implements PlayUpdateSign {
	...
	public GamePlayUpdateSign(PacketPlayOutUpdateSign packet){
		...
		try {
			...
			block 				   = Bukkit.getWorld(world.getWorldData().getName()).getBlockAt(position.getX(), position.getY(), position.getZ());
			...
			for(int i=0;i<nmsLines.length;i++){
				lines[i] = TextComponent.toLegacyText(fromChat(nmsLines[i]));
			}
		} catch(Exception e){}
	}
