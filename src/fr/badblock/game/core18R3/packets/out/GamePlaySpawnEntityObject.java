...
public class GamePlaySpawnEntityObject extends GameBadblockOutPacket implements PlaySpawnEntityObject {
	...
	public GamePlaySpawnEntityObject(PacketPlayOutSpawnEntity packet) {
		...
		try {
			...
			location = new Location(Bukkit.getWorlds().get(0), (double) reflector.getFieldValue("b") / 32.0D,
			...
			velocity = new Vector((double) reflector.getFieldValue("e") / 8000.0D,
					(double) reflector.getFieldValue("f") / 8000.0D, (double) reflector.getFieldValue("g") / 8000.0D);
