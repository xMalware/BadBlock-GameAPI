...
public class PlayerInteractListener extends BadListener {
	private void doCompassListener(PlayerInteractEvent e) {
		...
		} else if (GameAPI.getAPI().isCompassSelectNearestTarget()
		...
		if (e.getAction() == Action.RIGHT_CLICK_BLOCK || e.getAction() == Action.LEFT_CLICK_BLOCK) {
			if (ItemStackUtils.isValid(e.getItem()) && e.getItem().getType() == Material.BLAZE_ROD) {
				...
				if (e.getAction() == Action.RIGHT_CLICK_BLOCK) {
					...
					player.sendTranslatedMessage("commands.selection-first", e.getClickedBlock().getX(),
							e.getClickedBlock().getY(), e.getClickedBlock().getZ());
				} else {
					player.setFirstVector(new Vector3f(e.getClickedBlock().getLocation()));
					player.sendTranslatedMessage("commands.selection-second", e.getClickedBlock().getX(),
							e.getClickedBlock().getY(), e.getClickedBlock().getZ());
				}
