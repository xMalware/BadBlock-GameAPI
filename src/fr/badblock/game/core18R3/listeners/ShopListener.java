...
public class ShopListener extends BadListener {
	...
	/*
	 * @EventHandler public void onJoin(PlayerJoinEvent e){ for(Merchant
	 * merchant : GamePlugin.getInstance().getMerchants().values()){
	 * merchant.move((BadblockPlayer) e.getPlayer(),
	 * e.getPlayer().getLocation().clone().add(0, 64, 0),
	 ...
	 * GamePlugin.getInstance().getMerchants().values()){
	 * merchant.move((BadblockPlayer) e.getPlayer(), e.getFrom(), e.getTo()); }
	 * }
	 */

	/*
	 * @EventHandler public void onMove(PlayerTeleportEvent e){ for(Merchant
	 * merchant : GamePlugin.getInstance().getMerchants().values()){
	 * merchant.move((BadblockPlayer) e.getPlayer(), e.getFrom(), e.getTo()); }
	 * }
	 */
	...
	public void onCloseInventory(InventoryCloseEvent e) {
		...
		if (!inEdit.containsKey(e.getPlayer().getUniqueId())) {
			...
			merchant = inEdit.remove(e.getPlayer().getUniqueId());
