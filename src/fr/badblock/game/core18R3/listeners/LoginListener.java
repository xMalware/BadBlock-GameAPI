...
/**
 * Listener servant � remplac� la classe CraftPlayer par GameBadblockPlayer et �
 * demander � Ladder les informations joueur.
 * 
 * @author LeLanN
 */
public class LoginListener extends BadListener {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				for (Player player : Bukkit.getOnlinePlayers()) {
					...
					/*
					 * if(bp.isDisguised()){ bp.getDisguiseEntity().show(p); }
					 * else
					 */
	...
	public void onLogin(PlayerLoginEvent e) {
		...
		try {
			player = new GameBadblockPlayer((CraftServer) Bukkit.getServer(), (EntityPlayer) reflector.getReflected(),
					(GameOfflinePlayer) offlinePlayer);
