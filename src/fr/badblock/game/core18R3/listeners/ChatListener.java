...
public class ChatListener extends BadListener {
	...
	public void onChat(AsyncPlayerChatEvent e){
		...
		if(player.getBadblockMode() == BadblockMode.SPECTATOR){
			TranslatableString result = new TranslatableString("chat.spectator" + (custom == null ? "" : "." + custom), (LoginListener.l.contains(player.getName()) ? "§4♥ §r" : "") + player.getName(), player.getGroupPrefix(), e.getMessage(), player.getPlayerData().getLevel(), player.getGroupSuffix());
			...
			TranslatableString result = new TranslatableString("chat.team" + (custom == null ? "" : "." + custom), (LoginListener.l.contains(player.getName()) ? "§4♥ §r" : "") + player.getName(), player.getGroupPrefix(), player.getTeam().getChatName(), e.getMessage().substring(1), player.getPlayerData().getLevel(), player.getGroupSuffix());
			int i = new Random().nextInt(Integer.MAX_VALUE);
			while (messages.containsKey(i)) {
				i = new Random().nextInt(Integer.MAX_VALUE);
			}
			...
			messages.put(i, new ChatData(player.getName(), e.getMessage()));
			TranslatableString s = new TranslatableString("chat.player" + (custom == null ? "" : "." + custom), (LoginListener.l.contains(player.getName()) ? "§4♥ §r" : "") + player.getName(), player.getGroupPrefix(), team, e.getMessage(), player.getPlayerData().getLevel(), player.getGroupSuffix());
			TextComponent message = new TextComponent( GameAPI.i18n().get("chat.report_icon")[0] );
			message.setClickEvent( new ClickEvent( ClickEvent.Action.RUN_COMMAND, "/creport " + i) );
			message.setHoverEvent( new HoverEvent( HoverEvent.Action.SHOW_TEXT, new ComponentBuilder(GameAPI.i18n().get("chat.report_hover", player.getName())[0]).create() ) );
		...
		if((e.getMessage().startsWith("/t ") || e.getMessage().equalsIgnoreCase("/t") || e.getMessage().startsWith("/team ") || e.getMessage().equalsIgnoreCase("/team"))){
			...
			if (!team) {
				...
				if(!e.getMessage().contains(" ")) {
					...
					message.setClickEvent( new ClickEvent( ClickEvent.Action.RUN_COMMAND, "/creport " + i) );
					message.setHoverEvent( new HoverEvent( HoverEvent.Action.SHOW_TEXT, new ComponentBuilder(GameAPI.i18n().get("chat.report_hover", player.getName())[0]).create() ) );
					TranslatableString result = new TranslatableString("chat.team" + (custom == null ? "" : "." + custom), (LoginListener.l.contains(player.getName()) ? "§4♥ §r" : "") + player.getName(), player.getGroupPrefix(), player.getTeam().getChatName(), e.getMessage().replace(e.getMessage().split(" ")[0], ""), player.getPlayerData().getLevel());
					for(BadblockPlayer p : player.getTeam().getOnlinePlayers()){
						TextComponent textComponent = new TextComponent();
						String text = result.get((BadblockPlayer) p)[0];
						String coloredResult = "";
