...
public class DefaultKitContentManager implements PlayerKitContentManager {
	...
	public void give(JsonObject content, BadblockPlayer player) {
		...
		player.getInventory().setContents(
				configuration.getValueList("content", MapItemStack.class).getHandle().toArray(new ItemStack[0]));
		player.getInventory().setArmorContents(
				configuration.getValueList("armor", MapItemStack.class).getHandle().toArray(new ItemStack[0]));
	...
	private void prepareItem(BadblockPlayer player, ItemStack is, boolean allowDrop) {
		...
		if (is.getType().getMaxStackSize() > 1 || allowDrop)
			return; // que stuff quoi

		GameAPI.getAPI().createItemStackExtra(is).disallow(ItemAction.DROP).disallow(ItemAction.INVENTORY_DROP)
