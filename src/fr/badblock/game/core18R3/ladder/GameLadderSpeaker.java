...
public class GameLadderSpeaker implements LadderSpeaker, PacketHandler {
	...
	public GameLadderSpeaker(String ip, int port) throws IOException {
		...
		this.socketHandler = new SocketHandler(socket, Protocol.LADDER_TO_BUKKIT, Protocol.BUKKIT_TO_LADDER, this,
				false);
		...
		if (socketHandler == null || socketHandler.getSocket().isClosed()) {
			...
			new Thread() {
				...
				public void run() {
					try {
						Socket socket = new Socket(ip, port);
						socketHandler = new SocketHandler(socket, Protocol.LADDER_TO_BUKKIT, Protocol.BUKKIT_TO_LADDER,
								GameLadderSpeaker.this, false);
