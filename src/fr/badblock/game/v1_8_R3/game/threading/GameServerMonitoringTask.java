...
public class GameServerMonitoringTask extends GameServerTask {
	...
	public void run() {
		...
		double cpuLoad  = (getMXBean("getProcessCpuLoad").doubleValue() + getMXBean("getSystemCpuLoad").doubleValue()) * 100.0D;
		
		GameAPI.logColor("&b[GameServer &6DEDICATED&b] &7Instance" + (getOpenedServers() > 1 ? "s" : "") + ": " + getOpenedServers() + " | CPU: " + 
		MathsUtils.round(cpuLoad, 2) + "% | FREEMEM: " + MathsUtils.round(freeMemory, 2) + "MB");
