...
public class NMSBlaze extends EntityBlaze implements CustomCreature {
	@Getter
	...
	protected void regenerateAttributes(){
		...
		try {
			...
			for(Class<?> clazz : EntityBlaze.class.getDeclaredClasses()){
				if(clazz.getName().contains("PathfinderGoalBlazeFireball")){
					theClass = clazz;
				}
			}
			
			Constructor<?> constructor = theClass.getConstructor(EntityBlaze.class);
			constructor.setAccessible(true);
			
			this.goalSelector.a(4, (PathfinderGoal) constructor.newInstance(this));
		} catch (Exception e) {
			e.printStackTrace();
		}
