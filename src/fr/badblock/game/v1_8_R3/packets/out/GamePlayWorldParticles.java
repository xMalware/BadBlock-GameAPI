...
public class GamePlayWorldParticles extends GameBadblockOutPacket implements PlayWorldParticles {
	...
	public GamePlayWorldParticles(PacketPlayOutWorldParticles packet){
		...
		try {
			particle = new GameParticleEffect(ParticleEffectType.valueOf(((EnumParticle) reflector.getFieldValue("a")).name()))
							...
							.setOffset(new Vector(
									(float) reflector.getFieldValue("e"),
									(float) reflector.getFieldValue("f"),
									(float) reflector.getFieldValue("g")
							)).setData(ParticleData.load((int[]) reflector.getFieldValue("k")));
			location = new Location(Bukkit.getWorlds().get(0),
					(double) reflector.getFieldValue("b"),
					(double) reflector.getFieldValue("c"),
					(double) reflector.getFieldValue("d")
			);
	...
	public Packet<?> buildPacket() throws Exception {
		...
		} else if(particle.getType().getData() != null && !particle.getType().getData().equals(particle.getData().getClass())){
			new IllegalArgumentException("Bad data type for " + particle.getType() + " (need " + particle.getType().getData().getName() + ")");
		}
		...
		return new PacketPlayOutWorldParticles(type, particle.isLongDistance(), x, y, z, offsetX, offsetY, offsetZ, speed, amount, data);
	}
